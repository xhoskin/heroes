...
Calculate.prototype.damage = function(baseDmg, oursNum){
    ...
    if ( battle.attacker.attack >= battle.defenser.defense ) {
        var multDmg = 0.05;
    } else {
        var multDmg = 0.025;
    }
    ...
    var modDmg = ( battle.attacker.attack - battle.defenser.defense ) * multDmg;
...
Battlefield.prototype.refresh = function(){
    ...
    if ( calculate.damage(self.attacker.damageMin) != NaN )  {
        var min = calculate.damage(self.attacker.damageMin);
        var max  = calculate.damage(self.attacker.damageMax);
